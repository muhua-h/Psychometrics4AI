...
results_dir = Path(__file__).parent / 'study_2_likert_results'
json_files = glob.glob(str(results_dir / 'bfi_to_minimarker_*.json'))

if len(json_files) == 0:
    print(f"Error: No simulation results found in {results_dir}")
    print("Please run study_2_likert_multi_model_simulation.ipynb first.")
    exit(1)

print(f"Found {len(json_files)} Likert format model files:")
for f in json_files:
    print(f"  - {Path(f).name}")
    ...
    for idx, row in df.iterrows():
        ...
        for d in trait_by_domain:
            if trait_by_domain[d]:
                # Use sum instead of mean to match original Likert format
...
for json_file in json_files:
    model_name = Path(json_file).stem.replace('bfi_to_minimarker_', '').replace(
        '_temp1_0', '')

    print(f"\n--- Processing {model_name} (Likert Format) ---")
    ...
    sim_tda = sim_domains.loc[:n - 1, ['E', 'A', 'C', 'N', 'O']].reset_index(
        drop=True)

    # Check for NaN/inf values and clean data
    print(
        f"  Data shape: BFI-2={emp_bfi2.shape}, TDA={emp_tda.shape}, Sim={sim_tda.shape}")
    ...
    valid_mask = ~(sim_tda.isna().any(axis=1) | np.isinf(sim_tda.values).any(
        axis=1))
    ...
    def compute_corrs(arr1, arr2):
        ...
        for i, trait in enumerate(['E', 'A', 'C', 'N', 'O']):
            try:
                ...
                valid_idx = ~(
                            np.isnan(x) | np.isnan(y) | np.isinf(x) | np.isinf(
                        y))
...
print("\n" + "=" * 80)
print("DETAILED CONVERGENT VALIDITY RESULTS (LIKERT FORMAT)")
print("=" * 80)
...
print("\n" + "=" * 80)
print("LIKERT FORMAT MODEL COMPARISON SUMMARY")
print("=" * 80)
...
for model_name, results in model_results.items():
    ...
    print(
        f"{model_name:<15} {bfi_orig_avg:<10.3f} {bfi_sim_avg:<10.3f} {orig_sim_avg:<10.3f}")

# --- 7. Format comparison preparation ---
print("\n" + "=" * 80)
print("LIKERT FORMAT ANALYSIS COMPLETE")
print("=" * 80)
...
print("\nNext Steps:")
print(
    "1. Compare with expanded format results from study_2_expanded_convergent_analysis.py")
print("2. Run statistical tests for format differences")
print("3. Analyze domain-specific performance patterns")
...
for model_name, results in model_results.items():
    results_summary['summary_stats'][model_name] = {
        'bfi_orig_avg': float(np.nanmean(results['bfi_orig'])),
        'bfi_sim_avg': float(np.nanmean(results['bfi_sim'])),
        'orig_sim_avg': float(np.nanmean(results['orig_sim'])),
        'n_participants': int(results['n_participants'])
    }

# Save to JSON for easy comparison
output_path = Path(
    __file__).parent / 'study_2_likert_results' / 'likert_convergent_analysis_summary.json'
