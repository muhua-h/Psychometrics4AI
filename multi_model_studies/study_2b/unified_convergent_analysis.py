#!/usr/bin/env python3
"""
Unified Convergent Analysis for Study 2b - Multi-Model Multi-Format Simulation
...
def load_original_study3_data():
    ...
    if not data_path.exists():
        raise FileNotFoundError(f"Study 2b simulated data file not found: {data_path}")
    
    df = pd.read_csv(data_path)
    
    # The Study 2b data already has the correct domain scores (bfi_e, bfi_a, etc.)
    ...
    # For Study 2b, we use the same BFI-2 values as baseline since this is simulated data
...
def aggregate_minimarker_responses(df, format_type='likert'):
    ...
    # Use the exact same approach as the original Study 2b
    def reverse_score(score):
        return 10 - score

    def calculate_big_five_scores(df):
        # Exact mapping from original Study 2b process_json_bfi_miniMarker.ipynb
        dimensions = {
...
def main():
    ...
    logger.info("Starting unified convergent analysis for Study 2b (multi-format)...")
    
    # Load Study 2b simulated data
    try:
        simulated_data = load_original_study3_data()
    except FileNotFoundError as e:
        logger.error(f"Could not load Study 2b data: {e}")
        return
    ...
    logger.info("\n" + "="*70)
    logger.info("STUDY 2b CONVERGENT VALIDITY ANALYSIS SUMMARY")
    logger.info("="*70)
