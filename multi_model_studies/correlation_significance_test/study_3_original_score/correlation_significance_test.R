...
.path_from_this_script <- function() {
  ...
  return(normalizePath("multi_model_studies/correlation_significance_test/study_3_original_score/correlation_significance_test.R"))
}
...
normalize_model <- function(x) {
  x <- str_trim(x)
  # Remove "binary_" prefix if present
  x <- sub("^binary_", "", x)
  dplyr::case_when(
...
# Map any reasonable format spellings to exactly these four:
...
normalize_condition <- function(x) {
  ...
  xl <- tolower(x0)
  dplyr::case_when(
    xl %in% c("simple binary", "binary", "simple-binary", "binary_simple") ~ "Simple Binary",
    xl %in% c("elaborated binary", "elaborated-binary", "binary_elaborated")   ~ "Elaborated Binary",
    xl %in% c("expanded format", "expanded-format", "expanded")       ~ "Expanded Format",
...
required_cols <- c(
  "format", "model",
  "O_correlation", "C_correlation", "E_correlation", "A_correlation", "N_correlation"
)
...
tmp <- raw %>%
  mutate(
    Condition_chr = normalize_condition(format),
...
if (length(unknown_conditions) > 0) {
  warning(sprintf(
    "Found format values not in expected set; leaving as-is: %s",
...
correlations <- tmp %>%
  transmute(
    format = factor(Condition_chr, levels = condition_levels),
...
perform_test <- function(ai_corr, human_corr, domain, format, model) {
...
for (i in 1:nrow(correlations)) {
  format <- as.character(correlations$format[i])
  ...
  for (domain in domains) {
    ...
    results_list <- append(results_list, list(
      perform_test(ai_corr, human_corr, domain, format, model)
    ))
  }
}
...
results_df <- do.call(rbind, lapply(results_list, function(x) {
  data.frame(
    format = ifelse(is.null(x$format) || is.na(x$format), "Unknown", x$format),
