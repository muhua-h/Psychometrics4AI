#!/usr/bin/env python3
"""
Study 2b Generalized Multi-Model Risk-Taking Scenario Simulation

This script replicates the original Study 2b risk-taking simulation
...
def process_participant_risk_with_retry(participant_data, model, personality_format, temperature=0.0):
        ...
        if isinstance(response, str):
            ...
            if extracted_jsons:
                ...
                for json_obj in extracted_jsons:
                    is_valid, validation_msg = validate_risk_response(json_obj)
                    if is_valid:
                        return json_obj

                # If no valid JSON, return the first one with error info
                _, validation_msg = validate_risk_response(extracted_jsons[0])
            ...
            is_valid, validation_msg = validate_risk_response(response)
            if is_valid:
                return response
            else:
                return {"error": f"Invalid response format: {validation_msg}", "raw_response": response}
...
def process_participant_risk(participant_data, model, personality_format, temperature=0.0):
    """Process a single participant for risk-taking scenarios with fallback"""
    try:
        return process_participant_risk_with_retry(participant_data, model, personality_format, temperature)
    except Exception as e:
    ...
    try:
        ...
        for i in range(0, len(participants_data), batch_size):
            ...
            for participant in batch:
                result = process_participant_risk(participant, model, personality_format, temperature)
...
def run_risk_simulation(participants_data, model, personality_format, temperature, output_dir):
    """Run risk simulation for a single model and format"""
...
def retry_failed_participants(results, participants_data, model, personality_format, temperature):
    ...
    for idx in failed_indices:
        ...
        new_result = process_participant_risk(participant, model, personality_format, temperature)
...
def main():
    ...
    print("=== Study 2b Generalized Risk Simulation (Parallelized) ===")
    ...
    for format_name in personality_formats:
        ...
        risk_dir = format_dir / "risk"
        risk_dir.mkdir(exist_ok=True)
        format_dirs[format_name] = risk_dir
