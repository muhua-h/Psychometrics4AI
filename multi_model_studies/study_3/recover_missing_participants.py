...
from binary_baseline_prompt import get_binary_prompt, generate_binary_personality_description
...
def analyze_format_for_missing_participants(format_config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for json_file in json_files:
        ...
        model_name = file_stem.replace('bfi_to_minimarker_', '').replace('binary_', '')
...
def prepare_participant_data_for_format(participant_data: Dict[str, Any], format_type: str, data_row: pd.Series) -> Dict[str, Any]:
    """
...
def cleanup_duplicate_files(recovered_file_path: str, model_name: str, format_config: Dict[str, Any]):
    """
    ...
    for file_path in all_files:
        ...
        file_model_name = file_stem.replace('bfi_to_minimarker_', '').replace('binary_', '')
    ...
    for duplicate_file in model_files:
        try:
            ...
            duplicate_backup = Path(duplicate_file).with_suffix('.duplicate_backup.json')
            ...
            print(f"Warning: Could not remove duplicate file {duplicate_file}: {str(e)}")
...
def recover_participants_for_model(model_info: Dict[str, Any], 
    ...
    for idx in info['problematic_indices']:
        if idx < len(data):
            ...
            prepared_data = prepare_participant_data_for_format(participant_data, format_type, data_row)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Recover missing participants from Study 3 simulation results")
        ...
        for format_name, format_results in recovery_summary.items():
            ...
            for model_name, result in format_results.items():
                if result['status'] == 'success':
                    ...
                    if recovered > 0:
                        files_cleaned += 1
                        print(f"    ✓ Cleaned up duplicates, backup at: {result.get('backup_path', 'N/A')}")
                else:
