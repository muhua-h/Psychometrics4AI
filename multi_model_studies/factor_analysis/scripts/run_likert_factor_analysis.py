#!/usr/bin/env python3
"""
Run Factor Analysis for Missing Likert Format Data

This script uses the factor analysis framework to process Likert format simulation 
data for Studies 2 and 3, then converts the results to the standard CSV format.
"""
...
def run_likert_factor_analysis():
    """Run factor analysis for Likert format data in Studies 2 and 3"""
    
    base_dir = Path(__file__).parent
    results_dir = base_dir / "factor_analysis_results"
    factor_framework = FactorAnalysisFramework()
    
    logger.info("=== Running Likert Format Factor Analysis ===")
    
    # Study 2 Likert format
    logger.info("\n--- Study 2 Likert Format ---")
    study_2_likert_dir = base_dir / "study_2" / "study_2_likert_results"
    
    if study_2_likert_dir.exists():
        logger.info(f"Processing directory: {study_2_likert_dir}")
        ...
        if analysis_results:
            ...
            convert_factor_results_to_csv(analysis_results, "Study_2", "likert", results_dir)
        else:
    ...
    logger.info("\n--- Study 3 Likert Format ---")
    study_3_likert_dir = base_dir / "study_3" / "study_3_likert_results"
    
    if study_3_likert_dir.exists():
        logger.info(f"Processing directory: {study_3_likert_dir}")
