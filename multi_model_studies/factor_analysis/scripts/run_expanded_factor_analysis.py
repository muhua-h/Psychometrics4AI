#!/usr/bin/env python3
"""
Run Factor Analysis for Expanded Format Data

This script processes the expanded format factor analyses for Studies 2 and 3.
"""
...
def run_expanded_factor_analysis():
    """Run factor analysis for expanded format data in Studies 2 and 3"""
    
    base_dir = Path(__file__).parent
    results_dir = base_dir / "factor_analysis_results"
    factor_framework = FactorAnalysisFramework()
    
    logger.info("=== Running Expanded Format Factor Analysis ===")
    
    # Study 2 Expanded format
    logger.info("\n--- Study 2 Expanded Format ---")
    study_2_expanded_dir = base_dir / "study_2" / "study_2_expanded_results"
    
    if study_2_expanded_dir.exists():
        logger.info(f"Processing directory: {study_2_expanded_dir}")
        
        # Use the framework's batch analysis method
        analysis_results = factor_framework.analyze_all_model_results(
            study_2_expanded_dir, 
            pattern="*.json"
        )
        
        if analysis_results:
            logger.info(f"✅ Completed factor analysis for {len(analysis_results)} models")
            convert_factor_results_to_csv(analysis_results, "Study_2", "expanded", results_dir)
        else:
            logger.warning("❌ No results returned from factor analysis")
    else:
        logger.warning("❌ Study 2 Expanded results directory not found")
    
    # Study 3 Expanded format  
    logger.info("\n--- Study 3 Expanded Format ---")
    study_3_expanded_dir = base_dir / "study_3" / "study_3_expanded_results"
    
    if study_3_expanded_dir.exists():
        logger.info(f"Processing directory: {study_3_expanded_dir}")
        
        # Use the framework's batch analysis method
        analysis_results = factor_framework.analyze_all_model_results(
            study_3_expanded_dir, 
            pattern="*.json"
        )
        ...
        if analysis_results:
            ...
            convert_factor_results_to_csv(filtered_results, "Study_3", "expanded", results_dir)
        else:
            logger.warning("❌ No results returned from factor analysis")
    else:
        logger.warning("❌ Study 3 Expanded results directory not found")
    
    logger.info("\n=== Expanded Format Factor Analysis Complete ===")

def main():
    """Main execution function"""
    run_expanded_factor_analysis()
