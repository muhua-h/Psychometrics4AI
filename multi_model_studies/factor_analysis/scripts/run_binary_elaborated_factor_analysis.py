#!/usr/bin/env python3
"""
Run Factor Analysis for Binary Elaborated Format Data

This script processes the binary elaborated format factor analyses for Studies 2 and 3.
"""
...
def run_binary_elaborated_factor_analysis():
    """Run factor analysis for binary elaborated format data in Studies 2 and 3"""
    
    base_dir = Path(__file__).parent
    results_dir = base_dir / "factor_analysis_results"
    factor_framework = FactorAnalysisFramework()
    
    logger.info("=== Running Binary Elaborated Format Factor Analysis ===")
    
    # Study 2 Binary Elaborated format
    logger.info("\n--- Study 2 Binary Elaborated Format ---")
    study_2_binary_elaborated_dir = base_dir / "study_2" / "study_2_elaborated_binary_results"
    
    if study_2_binary_elaborated_dir.exists():
        logger.info(f"Processing directory: {study_2_binary_elaborated_dir}")
        
        # Use the framework's batch analysis method
        analysis_results = factor_framework.analyze_all_model_results(
            study_2_binary_elaborated_dir, 
            pattern="*.json"
        )
        
        if analysis_results:
            logger.info(f"✅ Completed factor analysis for {len(analysis_results)} models")
            convert_factor_results_to_csv(analysis_results, "Study_2", "binary_elaborated", results_dir)
        else:
            logger.warning("❌ No results returned from factor analysis")
    else:
        logger.warning("❌ Study 2 Binary Elaborated results directory not found")
    
    # Study 3 Binary Elaborated format  
    logger.info("\n--- Study 3 Binary Elaborated Format ---")
    study_3_binary_elaborated_dir = base_dir / "study_3" / "study_3_binary_elaborated_results"
    
    if study_3_binary_elaborated_dir.exists():
        logger.info(f"Processing directory: {study_3_binary_elaborated_dir}")
        
        # Use the framework's batch analysis method
        analysis_results = factor_framework.analyze_all_model_results(
            study_3_binary_elaborated_dir, 
            pattern="*.json"
        )
        ...
        if analysis_results:
            ...
            convert_factor_results_to_csv(filtered_results, "Study_3", "binary_elaborated", results_dir)
        else:
            logger.warning("❌ No results returned from factor analysis")
    else:
        logger.warning("❌ Study 3 Binary Elaborated results directory not found")
    
    logger.info("\n=== Binary Elaborated Format Factor Analysis Complete ===")

def main():
    """Main execution function"""
    run_binary_elaborated_factor_analysis()
