...
# Function to fit CFA model, compute fit measures, and calculate omega reliability for a single domain
fit_single_domain_model <- function(domain, items, data) {
  tryCatch({
    ...
    # Calculate omega reliability using compRelSEM
    omega <- tryCatch({
      compRelSEM(fit, return.total = TRUE)
    }, error = function(e) {
      # Fallback: try using psych package omega function
      tryCatch({
        omega_result <- psych::omega(data[, items])
        omega_result$omega.tot
      }, error = function(e2) {
        # If both methods fail, return NA
        NA
      })
    })
...
# Fit models, compute fit measures, and calculate omega reliability for each domain in Set 1
set1_results <- list()
for (domain in names(set1_domains)) {
  ...
  if ("error" %in% names(result)) {
    ...
    cat("\nOmega reliability:\n")
    print(result$omega_reliability)
  }
  cat("\n")
}
...
for (domain in names(set1_results)) {
  if ("error" %in% names(set1_results[[domain]])) {
    ...
    omega_val <- set1_results[[domain]]$omega_reliability
    if (is.na(omega_val)) {
      cat(paste(domain, ": Successfully fitted, Omega = NA (calculation failed)\n"))
    } else {
      cat(paste(domain, ": Successfully fitted, Omega =", round(omega_val, 3), "\n"))
    }
  }
}