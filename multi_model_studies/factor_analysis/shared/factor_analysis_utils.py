...
class FactorAnalysisFramework:
    ...
    # All Neuroticism items (8 items total)
    NEUROTICISM_FULL = ['Envious', 'Fretful', 'Jealous', 'Moody', 'Temperamental', 'Touchy', 'Relaxed', 'Unenvious']
    
    # Revised Neuroticism items (7 items, fallback)
    ...
    def analyze_simulation_results(self, result_path: Union[str, Path], 
                                 use_revised_neuroticism: bool = False) -> Dict[str, Any]:
        """
        ...
        for domain, domain_items in self.BIG_FIVE_DOMAINS.items():
            ...
            # Special handling for Neuroticism to try 8 items first, then fallback to 7
            if domain == 'Neuroticism':
                # Try all 8 items first
                all_items = self.NEUROTICISM_FULL
                logger.info(f"Analyzing {domain} with {len(all_items)} items (full set)")
                domain_result = self.fit_single_domain_cfa(domain, all_items, data_clean)
                
                # If full model fails, try fallback to 7 items
                if 'error' in domain_result or not domain_result.get('converged', False):
                    logger.info(f"Full Neuroticism model failed, trying 7-item version")
                    fallback_items = self.NEUROTICISM_REVISED
                    domain_result = self.fit_single_domain_cfa(domain, fallback_items, data_clean)
                    logger.info(f"Analyzing {domain} with {len(fallback_items)} items (fallback)")
                else:
                    logger.info(f"Neuroticism analysis successful with {len(all_items)} items")
            else:
