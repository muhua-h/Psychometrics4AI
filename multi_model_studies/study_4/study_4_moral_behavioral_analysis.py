#!/usr/bin/env python3
"""
Study 4 Multi-Model Moral Behavioral Validation Analysis
...
def load_human_data():
    """Load and preprocess human personality and moral response data"""
...
def run_regression_analysis(human_data, sim_data, model_name):
    """Run regression analysis comparing human personality traits to simulated moral responses"""
    ...
    for target in targets:
        if f'ethic_{target.split("_")[-1]}_1' in human_data.columns or target == 'moral_sum':
            if target == 'moral_sum':
                human_col = 'ethic_sum'
            else:
                # Map scenario names to human data columns
                scenario_map = {
                    'Confidential_Info': 'ethic_1_1',
                    'Underage_Drinking': 'ethic_2_1', 
                    'Exam_Cheating': 'ethic_3_1',
                    'Honest_Feedback': 'ethic_4_1',
                    'Workplace_Theft': 'ethic_5_1'
                }
...
def main():
    ...
    print("="*80)
    print("Study 4 Multi-Model Moral Behavioral Validation Analysis")
    print("="*80)
